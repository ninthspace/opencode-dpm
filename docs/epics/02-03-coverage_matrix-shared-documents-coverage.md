...
| 10 | ENVX2 | a design that assumes one is a design that runs on one host | The same round trip performed under a v1 stub context returns bytes identical to the one performed under a v2 stub context. | Story 4 | `[integration]` |  |
