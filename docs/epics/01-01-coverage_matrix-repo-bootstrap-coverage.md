...
| 23 | NFR5 | a dedicated CI sweep that imports every module under `src/` and `bin/` with plain `node` | The module sweep imports every file under `src/` and `bin/` with plain `node`, and every import resolves. | Story 6 | `[integration]` |  |
