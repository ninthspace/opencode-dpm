...
- ~~Given a stub v1 plugin context, the v1 registrar's `config` hook sets `mcp.dpm` to a local-server entry whose command runs the packaged executable. `[unit]`~~ **Superseded 2026-08-31T19:03:00.000Z**: The mechanism is unreachable from the shape v2 requires. Probes against opencode 1.18.25: v1's `config` hook arrives on the `Hooks` returned by the callable `server` export, and v1 only reaches that export when the module has NO default export — with a default present it calls `default.setup(context)` and ignores `server` entirely. v2 requires a default export object and never calls a callable one. Chris chose skills-without-tools under v1 on 2026-08-31, reaffirming it after FR2's text was put in front of him. Replaced by the criterion that the v1 registrar registers no server and says so loudly.
- ~~The local-server entry the v1 registrar builds is identical to the one the v2 registrar builds — same command array, same environment. `[unit]`~~ **Superseded 2026-08-31T19:03:00.000Z**: Superseded with 01M1C9C16CF1HKKNM8TZ304BF9: with no server registered under v1 there is no second local-server entry to compare against. The claim it was protecting — that one computation feeds both registrars rather than two that happen to agree — is not lost; story 2's `registrationInputs` makes it structural, and criterion 01M1C9B6J1PTGJ3P0P6XKCX68Z verifies it.
...
- ~~The v1 registrar appends the package's own skills directory to `config.skills.paths`, and discovery over that directory finds exactly twenty-three `SKILL.md` files, every one resolving to a path inside the installed package. `[unit]`~~ **Superseded 2026-08-31T21:45:00.000Z**: `config.skills.paths` is not the route, and does not need to be. Probing v1 1.18.25 found `skill.transform` on the object-route context, whose draft takes `{type:'embedded', skill}` sources carrying the computed skill rather than a path to it — a typed, published API where `skills.paths` is absent from both the SDK types and the live schema. The replacement criterion measures the same outcome, twenty-three skills each resolving inside the installed package, against the mechanism actually used.
- ~~`skills.paths` is written from exactly one module, and that module carries a marker naming the key as undocumented by OpenCode and saying what to do when it stops working. Control: a planted second write site fails the check. `[unit]`~~ **Superseded 2026-08-31T21:45:00.000Z**: There is no undocumented key to fence. NFR3 required `config.skills.paths` to be written from exactly one module and marked as unsupported; the embedded-source route means it is written from none. The requirement's premise dissolved rather than being satisfied, which is reported at the epic roll-up rather than claimed as coverage.
- ~~The post-registration check reports failure when given a host configuration that does not hold dpm's skills path, and passes when given one that does. `[unit]`~~ **Superseded 2026-08-31T21:45:00.000Z**: The post-registration check existed because a configuration key the host silently normalises away registers nothing while appearing to work. `skill.transform` fails at the call instead: the draft's `source` decodes what it is given and throws on a shape it does not accept, which is how the embedded variant was established in the first place. There is no silent-no-op failure mode left for a check to detect.
...
- ~~The skills module registers all twenty-three skills as `embedded` sources through `skill.transform`, each carrying a non-empty body and a `location` inside the installed package. Control: the source list is driven against a draft offering `source` and `list` and not `add`, so a registration written against the v2 API fails rather than passing against a double more generous than the host. `[integration]`~~ **Superseded 2026-09-12T14:15:00.000Z**: Names a mechanism that does not exist. 02-05 story 2 established that v1's plugin `Hooks` has no skill member of any kind and that the configuration layer strips the `plugins` key a second entry would have needed, then deleted that entry — so there is no `skill.transform` under v1 and no module to register `embedded` sources through it. The property the criterion measured, twenty-three skills each resolving inside the installed package, survives under the `skills` configuration key the user sets at install, and FR3 now records that route in as many words.
...
- ~~Every `dpm/shared/*.md` reference in every registered body has been rewritten to an absolute path that exists on disk, so a session reading a skill's conventions reaches the file rather than a path that resolves against nothing. Control: a body naming a shared file that is not in the package is refused at registration rather than registered with a broken path. `[integration]`~~ **Superseded 2026-09-12T14:15:00.000Z**: Names a registration-time transform the package no longer performs. ADR 02-01 replaced the `dpm/shared/*.md` path rewrite with the shared-document MCP tool, and 02-03 story 3 deleted `resolveSupportingPaths`. A host that reads `SKILL.md` verbatim off disk never hands the plugin the text to rewrite, which is the finding ENVX2 records; skills reach their conventions by asking the server for the document instead.
...
- ~~One plugin load under a stub v1 context produces a configuration holding both `mcp.dpm` and dpm's `skills.paths` entry, built from the same computed inputs rather than from two independent constructions. `[integration]`~~ **Superseded 2026-08-31T23:00:00.000Z**: Names `skills.paths`, which is not the route. v1 registers skills through `skill.transform`'s embedded sources, and the two registrations come from two modules rather than one load. Replaced by the criterion that asserts both entries resolving from one package root.
- ~~The same exported entry loaded under a stub v2 context produces the v2 registrations and writes no `config` at all. `[integration]`~~ **Superseded 2026-08-31T23:00:00.000Z**: Measures behaviour under a v2 context. v2 was dropped from this epic by explicit decision, so there is no second host to compare against. Replaced by the criterion that the registered MCP command is one a v1 host can actually spawn — the defect this story's integration check found.
