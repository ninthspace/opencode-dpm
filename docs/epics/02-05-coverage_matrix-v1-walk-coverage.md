...
| 9 | ENVR3 | installing the plugin into a throwaway project under each host and observing its MCP server reach connected state with the skills advertised | The plugin installed into a throwaway project under each host has its MCP server reach connected state with the skills advertised. | Story 2 | `[manual]` |  |
