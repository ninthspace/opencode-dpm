...
INSERT INTO "session" ("id", "skill", "phase", "state", "superseded_by", "created_at", "updated_at") VALUES ('8afd89dc-8e1d-4c30-959b-48255fc1b4f9', 'dpm:do', 'Epic 01-03 story 3 task 1 — retiring $ARGUMENTS across all 23 bodies', '{"test_command":"npm test","framework":"node:test","node":"24.20.0 via nvm, PATH must be exported first","tests":1047,"plan_approved":{"epic_01_03_story_1":"approved 2026-08-30; its four tasks are done"},"refactoring":{"01-02 story 1":"ran — consolidated four import-graph readings onto sources.js:staticImports","01-02 story 2":"ran — no consolidation found","01-02 story 3":"ran — extracted tests/support/package-tree.js","01-02 story 4":"ran — added sources.js:packageLock, moved plugin.test.js onto the shared packageManifest reader","01-02 story 5":"skipped — the only candidate was merging two host doubles whose difference is the point","01-03 story 1":"ran — extracted skills.js:toolMentions, the one definition of a callable tool mention, now shared by toolNames and body-reads.js:sites"},"scratch_project":{"config_key":"plugins is an ARRAY of {package, options}, not a map","package":"must name the entry FILE not the directory","plugin_shape":"a plugin module MUST export a default object with an `id` string, else the host logs SchemaError Missing key [default][id] and reports (failed)","service":"opencode2 service stop && start from a shell with node 24 first on PATH","cli":"mcp list and plugin list report nothing for a second or two after service start — run twice","registry_read":"ctx.skill/tool/mcp .transform(draft => draft.list()); /api/skill returns [] for plugin-registered skills, so the probe is the only reader","tool_exec":"draft.get(''dpm_x'').execute(input, ctx) works only with a REAL sessionID from ctx.session.create — a stub id gives ''Unable to execute''","permission":"permission.external_directory takes ask|allow|deny or a glob record; without it a non-interactive run auto-rejects the substituted conventions path","models":"mtplx local provider ConnectionRefused; free hosted models wander — one shelled into the real checkout. Do NOT hand one bash again."},"epic_01_03":{"retro_gate":"10 dispositions recorded against the epic","oracle":"~/.claude/plugins/cache/ninthspace-marketplace/dpm/0.7.0/skills/ — v0.7.0''s 23 bodies; spec now differs by exactly 3 lines under prefix reversal","remaining":"424 legacy tool refs, 22 Triggers-on descriptions, 101 /dpm: refs across 22 skills + shared/status-model.md; ralph names .claude/ralph-loop.local.md 5 times and has no v2 equivalent","slash":"Skill.Info.slash is catalog VISIBILITY, not a /name trigger. Invocation is the built-in `skill` tool with the exact case-sensitive id. Story 3''s criterion needs NO amendment.","callable_transition":"skills.js: CALLABLE=dpm_ from SERVER_NAME; LEGACY_CALLABLE; EITHER_CALLABLE; toolMentions(); bodiesOnLegacyForm() asserted in skill-pilot.test.js against the 22. Consumers: toolNames, ungated, body-reads.js sites, skill-do, skill-retrofit (x2), reachability.","story_1":"closed complete with the gates limb of criterion 1 unmet — user chose complete-and-record. Coverage 01M194YRXJ2T0WZT4EPWDWQ2YT verified; 01M194YPV1N00588YDCT47GW1Q left unverified."},"peer_message":{"from":"claude-code-marketplace-0c","when":"2026-08-30 during story 1","content":"upstream dpm changed two narration rules — stop narrating the story plan column''s raw value (do/ralph/epics), and shared/skill-conventions.md gains a Conversational Output subsection ''Saying what a row holds''","decision":"NOT applied — this port is a faithful port of v0.7.0 and the oracle diff is story 1''s own check. Surface to Chris as a separate decision, not folded into the mechanical rename."},"notes":{"story_status_note":"has a short length cap; task status_note does not"}}', NULL, '2026-08-30T11:09:12.357Z', '2026-08-30T17:38:06.985Z');
