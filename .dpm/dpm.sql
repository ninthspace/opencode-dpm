...
**And the finding that made the manifest work worth doing regardless.** The package OpenCode had actually installed from the GitHub specifier in epic 01-04 was on disk in the scratch cache: **418 files, 6.4 MB**, the whole repository — `docs/` with this project''s planning corpus, `tests/`, `scripts/`, `.dpm/` with the database dump, and `.claude-plugin/plugin.json`, a manifest addressed to Claude Code shipped inside an OpenCode package. There was no `files` field, and `private: true` from the vendoring commit would have blocked distribution outright. Both were invisible from the manifest alone and obvious the moment the installed artefact was opened — which is the same move as running the README''s commands rather than reading them, one epic earlier.', NULL, NULL, NULL, NULL, NULL, NULL);
INSERT INTO "observation" ("id", "retro_id", "retro_kind", "story_id", "quick_id", "quick_kind", "position", "text", "synthesis", "note", "library_doc_id", "library_doc_kind", "retired_at", "retired_reason") VALUES ('01M1A6THRW6SGNJ4PPJQE61K6J', NULL, NULL, '01M1957TECZSEEFTKG9DNVT7S7', NULL, NULL, 0, 'Three absence criteria, and the work that mattered was choosing the instrument rather than writing the checks.
